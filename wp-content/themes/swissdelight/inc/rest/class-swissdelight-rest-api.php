...
if ( ! class_exists( 'Swissdelight_Rest_API' ) ) {
	/**
	 * Rest API class with configuration
	 */
	class Swissdelight_Rest_API {
		private static $instance;
		private $namespace;

		public function __construct() {
			// Init variables
			$this->set_namespace( 'swissdelight/v1' );

			// Localize theme's main js script with rest variable
			add_filter( 'swissdelight_filter_localize_main_js', array( $this, 'localize_script' ) );
		...
		function localize_script( $global ) {
			...
			return apply_filters( 'swissdelight_filter_rest_api_global_variables', $global, $this->get_namespace() );
		}

		function register_rest_api_route() {
			$routes = apply_filters( 'swissdelight_filter_rest_api_routes', array() );
	...
	Swissdelight_Rest_API::get_instance();
}
