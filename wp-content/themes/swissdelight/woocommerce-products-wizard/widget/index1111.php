...
<script>
    jQuery(document).on('ajaxCompleted.wcpw', (e, instance, response, formData, options) => {
        ...
        if (response.stepId === "4") {
            var attributePaBoxVolume = "";
            if (typeof response.cart[0] !== 'undefined') {
                // console.log(response.cart[0].vn['attribute_pa_box-volume'].search('pcs'));
                if (response.cart[0].vn['attribute_pa_box-volume'].search('pcs') > -1) {
                    // var indexEl = response.cart[0].vn['attribute_pa_box-volume'].indexOf('pcs');
                    // var elements = response.cart[0].vn['attribute_pa_box-volume'].splice(indexEl);

                    attributePaBoxVolume = response.cart[0].vn['attribute_pa_box-volume'].match(/\d+/g);
                    console.log(attributePaBoxVolume)
                }
            }
            ...
            mathwidthLength = Math.round(widthLength * 100 / attributePaBoxVolume);
