(function ( $ ) {
	...
	var qodefSwiper = {
		...
		getOptions: function ( $holder, returnBreakpoints ) {
			...
			var slidesPerView1440 = sliderOptions.slidesPerView1440 !== undefined && sliderOptions.slidesPerView1440 !== '' ? parseInt( sliderOptions.slidesPerView1440, 10 ) : 5,
				...
				slidesPerView1024 = sliderOptions.slidesPerView1024 !== undefined && sliderOptions.slidesPerView1024 !== '' ? parseInt( sliderOptions.slidesPerView1024, 10 ) : 2,
				slidesPerView768  = sliderOptions.slidesPerView768 !== undefined && sliderOptions.slidesPerView768 !== '' ? parseInt( sliderOptions.slidesPerView768, 10 ) : 1,
			...
			if ( $holder.hasClass('qodef-fade-transition--enabled') ) {
				effect = 'fade';
			}
			...
			if ( this.holder.length ) {
				this.holder.each(
					function () {
						...
						} else if ( $thisPopup.hasClass( 'qodef-popup-video-item' ) ) {
							qodefMagnificPopup.initSingleImagePopup( $thisPopup );
						}
					}
				);
	...
	var qodefAppear = {
		init: function () {
			this.holder = $('.qodef-appear-animation--enabled, .qodef-custom-appear--left, .qodef-custom-appear--right, .qodef-custom-appear--bottom');
	...
	var qodefFloat = {
		init: function () {
			this.holder = $('.qodef-float-animation--enabled');

			if ( this.holder.length ) {
				this.holder.each( function () {
					var $holder    = $(this),
						distance   = $holder.data('float-distance'),
						smoothness = $holder.data('float-smoothness');

					if ( undefined === distance ) {
						distance = 30;
					}

					if ( undefined === smoothness ) {
						smoothness = 60;
					}

					$holder.attr('data-parallax', '{"y": ' + -Math.abs(distance) + ', "smoothness": ' + smoothness + '}');
	...
	var qodefModifyWooButtons = {
		init: function () {
			this.buttons = $('.single-product .single_add_to_cart_button, .single-product .button.wc-forward, .woocommerce-account .button, .woocommerce-cart .button, .woocommerce-checkout .button, .qodef-front-end-submit');

			if ( this.buttons.length ) {
				this.buttons.each( function () {
					var $thisButton = $( this );

					if ( !$thisButton.hasClass('qodef-borders--added') ) {
						$thisButton.addClass('qodef-borders--added');
						$thisButton.append('<span class="qodef-m-border--top-left"></span><span class="qodef-m-border--bottom-right"></span>');
					}
				});
			}
		}
	...
	qodef.qodefModifyWooButtons = qodefModifyWooButtons;
	...
	var qodefUncoveringSection = {
		init: function () {
			...
			if ( this.holder.length && !qodefCore.html.hasClass('touchevents') && qodef.windowWidth > 1024 ) {
				...
				qodefUncoveringSection.initialScroll();
		...
		setHeight: function ( $holder ) {
			...
			if ( sectionHeight > 0 ) {
				$('#qodef-page-content').css({ 'margin-top': sectionHeight });
		...
		initialScroll: function () {
			if ( 0 === $(window).scrollTop() ) {
				...
				$(window).one('mousewheel', function () {
					$('html, body').animate({
						scrollTop: $('#qodef-page-content').offset().top
					}, 1400, 'easeOutQuart');
...
(function ( $ ) {
	...
	var qodefFilter = {
		...
		setVisibility: function ( $holder, $item ) {
			...
			qodef.body.trigger(
				'swissdelight_trigger_load_more',
				[$holder, 1]
			);
		},
