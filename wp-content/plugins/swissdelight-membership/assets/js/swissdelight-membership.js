(function ( $ ) {
	...
	var qodefLoginModal = {
		...
		triggerShowModal: function ( $holder ) {
			...
			$( document.body ).on(
				'swissdelight_membership_trigger_login_modal',
				function () {
			...
			if ( $form.length ) {
				...
				$socialButton.find( '.qodef-m-social-login-btn' ).on(
					...
					function ( e ) {
						...
						$( document ).trigger(
							'swissdelight_membership_social_login_is_triggered',
		...
		triggerRequest: function ( $holder, socialNetwork, socialResponse ) {
			...
			var ajaxData = {
				...
				nonce: $holder.find( '#swissdelight-membership-ajax-' + $requestType + '-nonce' ).val()
			};
		...
		triggerForceLogin: function ( $holder, ajaxData, redirect ) {
			...
			ajaxData.nonce                = $holder.parent().find( '#swissdelight-membership-ajax-login-nonce' ).val();
	...
	var qodefLoginOpener = {
		...
		triggerClick: function ($holder) {
			...
			$opener.on('click', function (e) {
				...
				$(document.body).trigger('swissdelight_membership_trigger_login_modal');
			});
		}
	};
...
(function ($) {
	...
	$(document).on('swissdelight_membership_social_login_is_triggered', function (e, $modal, $form, social) {
	...
	var qodefFacebookLogin = {
		init: function () {
			...
			if (qodefFacebookLogin.fbIsAppIdSet()) {
				qodefFacebookLogin.fbAsyncInit(swissdelightMembershipGlobal.facebookAppId);
			}
		},
		fbIsAppIdSet: function () {
			return typeof swissdelightMembershipGlobal.facebookAppId !== 'undefined' && swissdelightMembershipGlobal.facebookAppId !== '';
		},
	...
	var qodefGoogleLogin = {
		init: function () {
			...
			if (qodefGoogleLogin.isAppIdSet()) {
				qodefGoogleLogin.asyncInit(swissdelightMembershipGlobal.googleAppId);
			}
		},
		isAppIdSet: function () {
			return typeof swissdelightMembershipGlobal.googleAppId !== 'undefined' && swissdelightMembershipGlobal.googleAppId !== '';
		},
