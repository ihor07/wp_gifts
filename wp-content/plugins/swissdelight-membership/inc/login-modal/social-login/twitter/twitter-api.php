...
if ( ! class_exists( 'SwissDelightMembershipTwitterApi' ) ) {
	/**
	 * Class SwissDelightMembershipTwitterApi
	 */
	class SwissDelightMembershipTwitterApi {
		/**
		 * @var $instance SwissDelightMembershipTwitterApi class object
		 */
		...
		/**
		 * @return SwissDelightMembershipTwitterApi
		 */
		 ...
		 *
		 * @see SwissDelightMembershipTwitterApi::generateSignatureBase()
		 */
		...
		public function obtainRequestToken() {
			...
			if ( is_wp_error( $response ) ) {
				...
				$responseObj->message = esc_html__( 'Internal WP error', 'swissdelight-membership' );
			} else {
				...
				if ( ! empty( $responseBody ) ) {
					...
					if ( ( is_array( $responseParsed ) && count( $responseParsed ) ) && ! empty( $responseParsed['oauth_token'] ) && ! empty( $responseParsed['oauth_token_secret'] ) ) {
						...
						if ( empty( $responseObj->redirectUrl ) ) {
							...
							$responseObj->message                  = esc_html__( 'Redirect URL could not be generated', 'swissdelight-membership' );
						} else {
							$responseObj->oauth_callback_confirmed = true;
						}
					} else {
						$responseObj->oauth_callback_confirmed = false;
						$responseObj->message                  = esc_html__( 'Couldn\'t connect with Twitter API', 'swissdelight-membership' );
					}
				}
			}
		...
		public function buildCurrentPageURI() {
			...
			return swissdelight_membership_get_membership_redirect_url();
		}
	}
}