...
if ( ! class_exists( 'SwissDelightMembershipPageTemplates' ) ) {
	class SwissDelightMembershipPageTemplates {
		...
		function __construct() {
			...
			$this->set_templates( array(
				'user-dashboard.php' => esc_html__( 'Qode User Dashboard', 'swissdelight-membership' )
			) );
			...
			add_filter( 'swissdelight_membership_filter_dashboard_template_name', array( $this, 'set_dashboard_template_name' ) );
			
			// Include dashboard page template content when user is logged in
			add_action( 'swissdelight_membership_action_after_user_dashboard_page_content', array( $this, 'include_dashboard_template_content' ) );
		}
		...
		public function get_template_path( $template ) {
			...
			if ( isset( $post ) && ! empty( $post ) && ! is_search() ) {
				...
				$file = SWISSDELIGHT_MEMBERSHIP_INC_PATH . '/general/page-templates/' . $page_template;
		...
		function include_dashboard_template_content() {
			...
			if ( is_user_logged_in() ) {
				swissdelight_membership_template_part( 'general', 'page-templates/parts/content' );
			}
		}
	}
	
	SwissDelightMembershipPageTemplates::get_instance();
}