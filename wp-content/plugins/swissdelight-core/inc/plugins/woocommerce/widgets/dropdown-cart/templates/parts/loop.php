		...
		if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
			...
			<div class="qodef-woo-dropdown-item qodef-e">
				...
				<div class="qodef-e-content">
					...
					<p class="qodef-e-quantity">
                        <?php echo sprintf(_n( '%s piece', '%s pieces',  esc_attr( $cart_item['quantity']) , 'swissdelight-core' ), esc_attr( $cart_item['quantity']))?>
					<?php echo sprintf( '<a href="%s" class="qodef-e-remove remove" aria-label="%s" data-product_id="%s" data-product_sku="%s">%s</a>', esc_url( wc_get_cart_remove_url( $cart_item_key ) ), esc_attr__( 'Remove this item', 'swissdelight-core' ), esc_attr( $product_id ), esc_attr( $_product->get_sku() ), qode_framework_icons()->get_specific_icon_from_pack( 'close', 'linea-icons' ) ); ?>
				</div>
			</div>
			<?php
		}
