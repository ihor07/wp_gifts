...
if ( ! function_exists( 'swissdelight_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function swissdelight_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'SwissDelightCore_Highlight_Shortcode';

		return $shortcodes;
	}

	add_filter( 'swissdelight_core_filter_register_shortcodes', 'swissdelight_core_add_highlight_shortcode' );
}

if ( class_exists( 'SwissDelightCore_Shortcode' ) ) {
	class SwissDelightCore_Highlight_Shortcode extends SwissDelightCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( SWISSDELIGHT_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'swissdelight_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'swissdelight-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'swissdelight-core' ) );
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'text_tag',
					'title'         => esc_html__( 'Text Tag', 'swissdelight-core' ),
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Disable Text Line Break', 'swissdelight-core' ),
					'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'swissdelight-core' ),
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Highlight Text Position', 'swissdelight-core' ),
					'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'swissdelight-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Color', 'swissdelight-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Background Color', 'swissdelight-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type'    => 'text',
					'name'          => 'link',
					'title'         => esc_html__( 'Link', 'swissdelight-core' ),
					'default_value' => '',
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return swissdelight_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
