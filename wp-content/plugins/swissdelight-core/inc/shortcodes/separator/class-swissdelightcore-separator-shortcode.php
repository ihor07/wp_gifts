...
if ( ! function_exists( 'swissdelight_core_add_separator_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function swissdelight_core_add_separator_shortcode( $shortcodes ) {
		$shortcodes[] = 'SwissDelightCore_Separator_Shortcode';

		return $shortcodes;
	}

	add_filter( 'swissdelight_core_filter_register_shortcodes', 'swissdelight_core_add_separator_shortcode', 9 );
}

if ( class_exists( 'SwissDelightCore_Shortcode' ) ) {
	class SwissDelightCore_Separator_Shortcode extends SwissDelightCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( SWISSDELIGHT_CORE_SHORTCODES_URL_PATH . '/separator' );
			$this->set_base( 'swissdelight_core_separator' );
			$this->set_name( esc_html__( 'Separator', 'swissdelight-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays separator with provided parameters', 'swissdelight-core' ) );
			...
			$this->set_option(
				array(
					'field_type' => 'select',
					'name'       => 'position',
					'title'      => esc_html__( 'Position', 'swissdelight-core' ),
					'options'    => array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'width',
					'title'      => esc_html__( 'Width (px or %)', 'swissdelight-core' ),
					'group'      => esc_html__( 'Style', 'swissdelight-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'thickness',
					'title'      => esc_html__( 'Thickness (px)', 'swissdelight-core' ),
					'group'      => esc_html__( 'Style', 'swissdelight-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'margin_top',
					'title'      => esc_html__( 'Margin Top (px or %)', 'swissdelight-core' ),
					'group'      => esc_html__( 'Style', 'swissdelight-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'margin_bottom',
					'title'      => esc_html__( 'Margin Bottom (px or %)', 'swissdelight-core' ),
					'group'      => esc_html__( 'Style', 'swissdelight-core' ),
				)
			);
		}
		...
		public function render( $options, $content = null ) {
			...
			return swissdelight_core_get_template_part( 'shortcodes/separator', 'templates/separator', '', $atts );
		}
