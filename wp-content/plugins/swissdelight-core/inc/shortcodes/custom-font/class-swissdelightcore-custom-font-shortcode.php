...
if ( ! function_exists( 'swissdelight_core_add_custom_font_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function swissdelight_core_add_custom_font_shortcode( $shortcodes ) {
		$shortcodes[] = 'SwissDelightCore_Custom_Font_Shortcode';

		return $shortcodes;
	}

	add_filter( 'swissdelight_core_filter_register_shortcodes', 'swissdelight_core_add_custom_font_shortcode' );
}

if ( class_exists( 'SwissDelightCore_Shortcode' ) ) {
	class SwissDelightCore_Custom_Font_Shortcode extends SwissDelightCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'swissdelight_core_filter_custom_font_layouts', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( SWISSDELIGHT_CORE_SHORTCODES_URL_PATH . '/custom-font' );
			$this->set_base( 'swissdelight_core_custom_font' );
			$this->set_name( esc_html__( 'Custom Font', 'swissdelight-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays custom font with provided parameters', 'swissdelight-core' ) );
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'title',
					'title'      => esc_html__( 'Title Text', 'swissdelight-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					...
					'name'        => 'font_size_1366',
					'title'       => esc_html__( 'Font Size', 'swissdelight-core' ),
					'description' => esc_html__( 'Set responsive style value for screen size 1366', 'swissdelight-core' ),
					'group'       => esc_html__( 'Screen Size 1366', 'swissdelight-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					...
					'name'        => 'font_size_1024',
					'title'       => esc_html__( 'Font Size', 'swissdelight-core' ),
					'description' => esc_html__( 'Set responsive style value for screen size 1024', 'swissdelight-core' ),
					'group'       => esc_html__( 'Screen Size 1024', 'swissdelight-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					...
					'name'        => 'font_size_768',
					'title'       => esc_html__( 'Font Size', 'swissdelight-core' ),
					'description' => esc_html__( 'Set responsive style value for screen size 768', 'swissdelight-core' ),
					'group'       => esc_html__( 'Screen Size 768', 'swissdelight-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					...
					'name'        => 'font_size_680',
					'title'       => esc_html__( 'Font Size', 'swissdelight-core' ),
					'description' => esc_html__( 'Set responsive style value for screen size 680', 'swissdelight-core' ),
					'group'       => esc_html__( 'Screen Size 680', 'swissdelight-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public function render( $options, $content = null ) {
			...
			return swissdelight_core_get_template_part( 'shortcodes/custom-font', 'variations/' . $atts['layout'] . '/templates/custom-font', '', $atts );
		}
		...
		private function set_responsive_styles( $atts ) {
			...
			foreach ( $screen_sizes as $screen_size ) {
				...
				if ( ! empty( $styles ) ) {
					add_filter( 'swissdelight_core_filter_add_responsive_' . $screen_size . '_inline_style_in_footer', function ( $style ) use ( $unique_class, $styles ) {
