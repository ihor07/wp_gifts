...
class SwissDelightCore_Dashboard_Import {
	/**
	...
	/**
	 * @return SwissDelightCore_Dashboard_Import
	 */
	...
	public function set_import_url() {
		$params = SwissDelightCore_Dashboard::get_instance()->get_import_params();
				...
				switch ( $_POST['options']['action'] ) :
					...
					case 'rev-slider':
						if ( qode_framework_is_installed( 'revolution-slider' ) ) {
							...
							$this->set_message( esc_html__( 'Revolution Slider isn\'t installed', 'swissdelight-core' ) );
						}
						break;
					case 'content':
						...
						if ( $content_start ) {
							if ( ! SwissDelightCore_Dashboard::get_instance()->check_purchase_code( $_POST['options']['demo'] ) ) {
								qode_framework_get_ajax_status( 'error', esc_html__( 'Please don\'t try to hack me. Purchase code registered is not valid', 'swissdelight-core' ) );
								exit;
							}
						}
	...
	function file_content( $path ) {
		...
		if ( 200 !== intval( $response_code ) ) {
			$this->set_message( $response['response']['message'] . ' ' . esc_html__( 'Please contact support', 'swissdelight-core' ) );
	...
	public function import_widgets( $demo ) {
		...
		if ( $widgets_content ) {
			foreach ( (array) $widgets_content['widgets'] as $swissdelight_widget_id => $swissdelight_widget_data ) {
				$swissdelight_widget_data = $this->fix_nav_menu_widgets( $swissdelight_widget_data, $swissdelight_widget_id );
				update_option( 'widget_' . $swissdelight_widget_id, $swissdelight_widget_data );
			}

			$ws = $this->import_sidebars_widgets( $widgets );

			if ( $ws ) {
				$this->set_message( esc_html__( 'Widgets are set for proper sidebar', 'swissdelight-core' ) );
				$this->set_data( 'type', 'options' );
				$this->set_status( 'success' );
			}
		}
	...
	function fix_nav_menu_widgets( $widget_data, $widget_id ) {
		...
		if ( 'nav_menu' === $widget_id ) {
			$imported_terms = get_transient( 'swissdelight_core_imported_terms' );
	...
	public function import_sidebars_widgets( $file ) {
		$swissdelight_sidebars = get_option( 'sidebars_widgets' );
		unset( $swissdelight_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$swissdelight_sidebars = array_merge( (array) $swissdelight_sidebars, (array) $data['sidebars'] );
			unset( $swissdelight_sidebars['wp_inactive_widgets'] );
			$swissdelight_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $swissdelight_sidebars );
			$swissdelight_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $swissdelight_sidebars );
	...
	public function import_options( $file ) {
		...
		$current_options = get_option( SWISSDELIGHT_CORE_OPTIONS_NAME );

		if ( $options ) {
			if ( $current_options != $options ) {
				$result = update_option( SWISSDELIGHT_CORE_OPTIONS_NAME, $options );

				if ( $result ) {
					$this->update_options_after_import( $file );
					$this->set_status( 'success' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'Options imported successfully', 'swissdelight-core' ) );

					$this->update_options_after_import( $file );

				} else {
					$this->set_status( 'error' );
					$this->set_message( esc_html__( 'Problem occurred during options import', 'swissdelight-core' ) );
				}
			} else {
				$this->set_status( 'success' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'Options are already imported', 'swissdelight-core' ) );
			}
		}
	}
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids       = get_transient( '_swissdelight_core_imported_posts' );
		$fields_status = true;
		...
		if ( $pages ) {
			...
			if ( ! $fields_status ) {
				$this->set_status( 'error' );
				$this->set_message( esc_html__( 'Problem occurred during settings pages import', 'swissdelight-core' ) );
			} else {
				$this->set_status( 'success' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'Settings pages imported successfully', 'swissdelight-core' ) );
			}
		} else {
			$this->set_status( 'error' );
			$this->set_message( esc_html__( 'File doesn\'t exist', 'swissdelight-core' ) );
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if ( $menus_data ) {
			...
			$this->set_status( 'success' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Menus set for proper locations', 'swissdelight-core' ) );
		} else {
			$this->set_status( 'error' );
			$this->set_message( esc_html__( 'Problem during menus location set', 'swissdelight-core' ) );
		}
	}

	public function import_content( $file, $xml, $attachments, $post_id ) {
		ob_start();
		require_once( SWISSDELIGHT_CORE_INC_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
		...
		$swissdelight_import = new WP_Import();
		set_time_limit( 0 );

		$swissdelight_import->fetch_attachments = $attachments;
		$returned_value                     = $swissdelight_import->import( $file . $xml );

		if ( is_wp_error( $returned_value ) ) {
			$this->set_status( 'error' );
			$this->set_data( 'type', 'content' );
			$this->set_data( 'xml', $xml );
			$this->set_message( esc_html__( 'An error occurred during content import', 'swissdelight-core' ) );
		} else {
			$this->set_status( 'success' );
			$this->set_data( 'type', 'content' );
			$this->set_data( 'posts', $this->imported_posts );
			$this->set_message( esc_html__( 'File imported successfully', 'swissdelight-core' ) . ' ' . $xml );
		}
	...
	public function rev_sliders() {
		$rev_sldiers = array(
			...
			'landing-4.zip',
			'cake-shop.zip',
			'chocolaterie-dark.zip',
			'chocolaterie-dark-pralines.zip',
		...
		if ( $files_created ) {
			...
			if ( class_exists( 'RevSliderSliderImport' ) ) {
				...
				if ( ! empty( $import_instance ) && ! is_wp_error( $import_instance ) ) {
					...
					foreach ( $demo_rev_sliders as $demo_rev_slider ) {
						...
						if ( ! in_array( str_replace( '.zip', '', $demo_rev_slider ), $rev_sliders, true ) ) {
							...
							if ( ! $rev_slider_result['success'] ) {
								$this->set_status( 'error' );
								$this->set_message( esc_html__( 'Error while importing rev sliders', 'swissdelight-core' ) );
								exit;
							}
						}
					}
					...
					$this->set_status( 'success' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'Rev sliders imported successfully', 'swissdelight-core' ) );
				} else {
					$this->set_status( 'error' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'RevSliderSliderImport instance is empty', 'swissdelight-core' ) );
				}
			} else {
				$this->set_status( 'error' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'RevSliderSliderImport class doesn\'t exist', 'swissdelight-core' ) );
			}
		} else {
			$this->set_status( 'error' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Files don\'t exist', 'swissdelight-core' ) );
		}
	}
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( SWISSDELIGHT_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( SWISSDELIGHT_CORE_OPTIONS_NAME, $new_global_values );
		}
	}
	...
	function get_noimage_url( $origin_img_url ) {
		...
		return SWISSDELIGHT_CORE_ASSETS_URL_PATH . '/img/' . $noimage_fname;
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && ! empty( $_POST ) && ! empty( $_POST['options']['demo'] ) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'qodef_cd_import_nonce' ) ) {
				...
				$html = swissdelight_core_get_template_part( 'core-dashboard/sub-pages/import/templates', 'pages-list', '', array( 'pages' => $pages ) );

				if ( $pages ) {
					qode_framework_get_ajax_status( 'success', '', $html );
				} else {
					qode_framework_get_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'swissdelight-core' ), '' );
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = SwissDelightCore_Dashboard_System_Info_Page::get_instance()->get_system_info();
...
SwissDelightCore_Dashboard_Import::get_instance();
