...
$masonry_classes   = '';
$number_of_columns = swissdelight_core_get_post_value_through_levels( 'qodef_portfolio_columns_number' );
$masonry_classes  .= ! empty( $number_of_columns ) ? ' qodef-col-num--' . $number_of_columns : ' qodef-col-num--3';

$space_between_items = swissdelight_core_get_post_value_through_levels( 'qodef_portfolio_space_between_items' );
...
if ( ! empty( $portfolio_media ) ) { ?>
	<div class="qodef-e qodef-grid qodef-layout--masonry qodef-items--fixed qodef-responsive--predefined qodef--no-bottom-space <?php echo esc_attr( $masonry_classes ); ?>">
		<div class="qodef-grid-inner clear qodef-magnific-popup qodef-popup-gallery">
			...
			swissdelight_core_theme_template_part( 'masonry', 'templates/sizer-gutter', '', 'masonry' );
			...
			foreach ( $portfolio_media as $media ) {
				...
				swissdelight_core_template_part( 'post-types/portfolio', 'templates/parts/media/media', $type . '-masonry', $params );
			}
			?>
		</div>
	</div>
