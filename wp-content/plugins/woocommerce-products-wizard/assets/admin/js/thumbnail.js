...
(function (root, factory) {
  ...
  Plugin.prototype.initEventListeners = function () {
    var _this = this;
    // set thumbnail
    this.$element.on('click', '[data-component~="wcpw-thumbnail-set"]', function (event) {
      event.preventDefault();
      return _this.openModal();
    });

    // remove thumbnail
    this.$element.on('click', '[data-component~="wcpw-thumbnail-remove"]', function (event) {
      event.preventDefault();
  ...
  Plugin.prototype.openModal = function () {
    ...
    this.modalFrame.on('select', function () {
      return _this2.modalFrame.state().get('selection').map(function (attachment) {
        ...
        _this2.$element.trigger('selected.thumbnail.wcpw', [_this2, attachment]);
        return attachment;
      });
    });
  ...
  $document.on('init.thumbnail.wcpw', function () {
    return init();
  });
});